...
export default function DetalleJugador() {
  ...
  useEffect(() => {
    ...
    // Obtener jugador
    fetch(`http://portuense-manager:8000/api/jugadores/${id}/`, {
      headers: {
    ...
    // Obtener comentarios
    fetch(`http://portuense-manager:8000/api/comentarios-jugador/jugador/${id}/`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  const eliminarComentario = (comentarioId) => {
    ...
    if (window.confirm("¿Seguro que quieres eliminar este comentario?")) {
      fetch(`http://portuense-manager:8000/api/comentarios-jugador/${comentarioId}/`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
